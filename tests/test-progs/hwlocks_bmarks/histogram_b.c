...
void* thread_fct(void* ptr) {
    ...
    for (counter = 0; counter < args->input_data_length; counter++) {
        pthread_mutex_lock(&args->mut[args->input_data[counter]]);
        args->local_output_data[args->input_data[counter]]++;
        pthread_mutex_unlock(&args->mut[args->input_data[counter]]);
    }
    pthread_exit(NULL);
}

void compute_histogram_b(long long ** output_data, unsigned char * input_data, long long input_data_length, int number_buckets, int number_threads)
{
    ...
    *output_data = local_output_data;
    ...
    pthread_mutex_t* mut = malloc(sizeof(pthread_mutex_t) * number_buckets); 
    for (counter_local = 0; counter_local < number_buckets; counter_local++) {
        pthread_mutex_init(&mut[counter_local], NULL);
    ...
    for (int i = 0; i < number_threads; i++) {
        ...
        thread_args[i].local_output_data = local_output_data;
...
int main(int argc, char * argv[], char**envp)
{
    ...
    compute_histogram_b(&histogram_array, data_array, atoi(argv[2]), atoi(argv[3])+1, atoi(argv[4]));
    // Get the ending time
