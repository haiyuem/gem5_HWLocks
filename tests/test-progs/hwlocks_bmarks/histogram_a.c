...
void* thread_fct(void* ptr) {
    ...
    for (counter = 0; counter < args->input_data_length; counter++) {
        pthread_mutex_lock(args->l);
        args->local_output_data[args->input_data[counter]]++;
        pthread_mutex_unlock(args->l);
    }
    pthread_exit(NULL);
}

void compute_histogram_a(long long ** output_data, unsigned char * input_data, long long input_data_length, int number_buckets, int number_threads)
{
    ...
    *output_data = local_output_data;
    ...
    pthread_mutex_t l; // mutex for output array
...
int main(int argc, char * argv[], char**envp)
{
    ...
    compute_histogram_a(&histogram_array, data_array, atoi(argv[2]), atoi(argv[3])+1, atoi(argv[4]));
    // Get the ending time
